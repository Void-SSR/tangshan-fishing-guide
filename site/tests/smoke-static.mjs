...
async function main() {
  ...
  assert.ok(homeHtml.includes("今天怎么钓，先从这里选"), "首页缺少新的出发型提示");
  assert.ok(homeHtml.includes("先选海岸钓还是出海钓，再进对应指南找点。"), "首页缺少新的分流说明");
  assert.ok(homeHtml.includes("打开后直接看当前模式下的钓点、窗口、风险和实战建议"), "首页缺少新的辅助说明");
  assert.ok(!homeHtml.includes("<base href="), "首页仍残留动态 base 注入");
  assert.ok(homeHtml.includes("进入海岸钓指南"), "首页缺少海岸钓指南入口");
  assert.ok(homeHtml.includes("进入出海钓指南"), "首页缺少出海钓指南入口");
  assert.ok(homeHtml.includes("17 个钓点"), "首页缺少基础概览信息");
  assert.ok(homeHtml.includes("海鲈 / 黑头 / 鲅鱼 / 梭鱼 / 六线鱼 / 黑鲷"), "首页缺少扩展后的海岸钓鱼种");
  assert.ok(homeHtml.includes("鲅鱼 / 海鲈 / 黑头 / 真鲷 / 牙鲆 / 半滑舌鳎"), "首页缺少扩展后的出海钓鱼种");
  assert.equal((homeHtml.match(/class="mode-card"/g) || []).length, 2, "首页模式卡片数量不正确");

  assert.ok(shoreHtml.includes("海岸钓"), "海岸钓列表页缺少标题");
  assert.ok(shoreHtml.includes("模式内钓点列表"), "海岸钓列表页缺少新的层级说明");
  assert.ok(shoreHtml.includes("以下点位默认按综合推荐顺序排列。"), "海岸钓列表页缺少新的指导说明");
  assert.ok(shoreHtml.includes("第一梯队"), "海岸钓列表页缺少梯队标题");
  assert.ok(shoreHtml.includes("优先看这些点"), "海岸钓列表页缺少梯队引导语");
  assert.ok(shoreHtml.includes("适合谁先看"), "海岸钓列表页缺少比较信号");
  ...
  assert.ok(boatHtml.includes("模式内钓点列表"), "出海钓列表页缺少新的层级说明");
  assert.ok(boatHtml.includes("以下海域默认按综合推荐顺序排列。"), "出海钓列表页缺少新的指导说明");
  assert.ok(boatHtml.includes("第一梯队"), "出海钓列表页缺少梯队标题");
  assert.ok(boatHtml.includes("优先看这些点"), "出海钓列表页缺少梯队引导语");
  assert.ok(boatHtml.includes("适合谁先看"), "出海钓列表页缺少比较信号");
  ...
  assert.ok(sampleDetail.includes("一句话判断"), "详情页缺少一句话判断");
  assert.ok(!sampleDetail.includes("<base href="), "详情页仍残留动态 base 注入");
  assert.ok(sampleDetail.includes("信息来源与更新时间"), "详情页缺少来源与更新时间");
  assert.ok(sampleDetail.includes("待复核提示"), "详情页缺少待复核提示");
  assert.ok(sampleDetail.includes("查看核验与图片说明"), "详情页缺少核验与图片说明入口");
  assert.ok(sampleDetail.includes("图片说明：优先观察"), "详情页缺少主图说明");
  assert.ok(sampleDetail.includes("怎么钓"), "详情页首屏缺少怎么钓提示");
  ...
  for (const spot of siteData.spots) {
    ...
    assert.ok(detailHtml.includes("继续看下一个推荐点"), `详情页缺少下一推荐按钮: ${spot.id}`);
    assert.ok(detailHtml.includes("查看核验与图片说明"), `详情页缺少核验与图片说明入口: ${spot.id}`);
    assert.ok(detailHtml.includes("图片说明：优先观察"), `详情页缺少主图说明: ${spot.id}`);
